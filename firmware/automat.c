...
uint8_t mat_pir_spot_status, mat_pir_spot;
uint8_t mat_pir_room_status, mat_pir_room;
...
void mat_init(){
	...
	mat_pir_spot_status = 0;
	mat_pir_spot = 0;
	mat_pir_room_status = 0;
	mat_pir_room = 0;
	
	MAT_DOOR_PORT |= (1<<MAT_DOOR); //PULL-UP
	//no pullups for PIR sensors
...
void mat_check(){
	...
	if(mat_pir_spot_status==255){
		if(mat_pir_spot == 0){
			mat_pir_spot = 1;	//just opened
			char tmp[20];
			strcpy(tmp, "/o/p/0/");	//out,door
			itoa(mat_pir_spot,tmp+strlen(tmp),10);
			strcpy(tmp+strlen(tmp), "\r\n");
			uart_puts(tmp);
		}
	}
	...
	if(mat_pir_room_status==255){
		if(mat_pir_room == 0){
			mat_pir_room = 1;	//just opened
			char tmp[20];
			strcpy(tmp, "/o/p/1/");	//out,door
			itoa(mat_pir_room,tmp+strlen(tmp),10);
			strcpy(tmp+strlen(tmp), "\r\n");
			uart_puts(tmp);
		}
	}
